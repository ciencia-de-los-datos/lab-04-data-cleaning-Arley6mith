...
def clean_data():
    ...
    df["fecha_de_beneficio_1"] = pd.to_datetime(
        df.fecha_de_beneficio, format = "%Y/%m/%d", errors ="coerce")
    df["fecha_de_beneficio_2"] = pd.to_datetime(
        df.fecha_de_beneficio, format = "%d/%m/%Y", errors ="coerce")
    df["fecha_de_beneficio"] = df.apply(
    ...
    df= df.sort_values(
        by = ["sexo", "tipo_de_emprendimiento",
              "idea_negocio", "barrio",
              "estrato", "comuna_ciudadano",
              "fecha_de_beneficio",
